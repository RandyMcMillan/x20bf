...
class HiddenService:
    ...
    def __init__(self, onion_address, descriptor_cookie=None, auth_type=AuthType.No):
        ...
        self._onion_identity_pk = (
            curve25519_public_from_bytes(onion_identity_pk)
            if onion_identity_pk
            else None
        )
        if self._onion_identity_pk:
            raise Exception("v3 onion hidden service not supported yet")
        self._descriptor_cookie = (
            b64decode(descriptor_cookie) if descriptor_cookie else None
        )
    ...
    def normalize_onion(onion_address):
        ...
        if (
            len(onion_address) != HiddenService.REND_SERVICE_ID_LEN_BASE32
            and len(onion_address) != HiddenService.HS_SERVICE_ADDR_LEN_BASE32
        ):
    ...
    def parse_onion(onion_address):
        ...
        if len(onion_address) == HiddenService.REND_SERVICE_ID_LEN_BASE32:
            ...
            assert (
                len(permanent_id) == HiddenService.REND_SERVICE_ID_LEN
            ), "You must specify valid V2 onion hostname"
...
class HiddenServiceConnector:
    ...
    def get_responsibles_dir(self, hidden_service):
        for i, responsible_router in enumerate(
            self._consensus.get_responsibles(hidden_service)
        ):
            replica = 1 if i >= 3 else 0
            yield ResponsibleDir(
                responsible_router, replica, self._circuit, self._consensus
            )
...
class EncPointsBuffer:
    ...
    def _decrypt_basic(self, descriptor_cookie):
        ...
        entries_len = (
            block_count
            * self.REND_BASIC_AUTH_CLIENT_MULTIPLE
            * self.REND_BASIC_AUTH_CLIENT_ENTRY_LEN
        )
        assert (
            len(self._crypted_data) > 2 + entries_len + self.CIPHER_IV_LEN
        ), "Size of crypted data too small"
        iv = self._crypted_data[2 + entries_len : 2 + entries_len + self.CIPHER_IV_LEN]
        client_id = sha1(descriptor_cookie + iv)[:4]
        session_key = self._get_session_key(
            self._crypted_data[2 : 2 + entries_len], descriptor_cookie, client_id
        )
    ...
    def _decrypt_stealth(self, descriptor_cookie):
        assert (
            len(self._crypted_data) > 2 + self.CIPHER_IV_LEN
        ), "Size of encrypted data is too small"
...
class ResponsibleDir:
    ...
    def get_introductions(self, hidden_service):
        ...
        for intro_point in self._get_intro_points(
            response, hidden_service.descriptor_cookie
        ):
            yield intro_point
    ...
    def _fetch_descriptor(self, descriptor_id):
        ...
        with self._circuit.create_new_circuit(
            extend_routers=[self._router]
        ) as directory_circuit:
            ...
            with directory_circuit.create_dir_client() as dir_client:
                ...
                if status != 200:
                    logger.error(
                        "No valid response from hsdir. Status = %r. Body: %r",
                        status,
                        response,
                    )
    ...
    def _info_to_router(self, intro_point_info):
        onion_router = self._consensus.get_router(
            intro_point_info["introduction_point"]
        )
    ...
    def _get_intro_points(self, response, descriptor_cookie):
        ...
        if (
            intro_points_raw[0] == AuthType.Basic
            or intro_points_raw[0] == AuthType.Stealth
        ):
...
class IntroductionPoint:
    ...
    def connect(self, hidden_service, rendezvous_cookie):
        ...
        with self._circuit.create_waiter(CellRelayRendezvous2) as w:
            ...
            with self._circuit.create_new_circuit(
                extend_routers=[self._introduction_router]
            ) as intro_circuit:
