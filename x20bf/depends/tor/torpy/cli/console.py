...
def main():
    ...
    parser.add_argument(
        ...
        nargs=2,
        action="append",
        help="set some http header",
    )
    parser.add_argument(
        "--auth-data",
        nargs=2,
        action="append",
        help="set auth data for hidden service authorization",
    )
    parser.add_argument("--log-file", default=None, help="log file path")
    parser.add_argument(
        "--requests-lib",
        dest="request_func",
        default=urllib_request,
        action="store_const",
        const=requests_request,
        help="use requests library for making requests",
    )
    parser.add_argument(
        "-v", "--verbose", default=0, help="enable verbose output", action="count"
    )
