...
from torpy.crypto_common import b64decode, sha3_256
from torpy.documents.basics import TorDocument, TorDocumentObject
from torpy.documents.items import (
    Item,
    ItemDate,
    ItemEnum,
    ItemInt,
...
class ItemSignature(ItemMulti):
    ...
    def _parse_args(line, *_):
        ...
        if len(splits) == 2:
            return {
                "algorithm": "sha1",
                "identity": splits[0],
                "signing_key_digest": splits[1],
            }
        else:
            return {
                "algorithm": splits[0],
                "identity": splits[1],
                "signing_key_digest": splits[2],
            }
    ...
    def __init__(self, keyword):
        super().__init__(
            keyword,
            "signature",
            parse_func=ItemSignature._parse_args,
            out_name="signatures",
            as_list=True,
        )
...
class DirSourceObject(TorDocumentObject):
    ...
    START_ITEM = Item(
        ...
        parse_args=[
            " ",
            ["nickname", "fingerprint", "hostname", "address", "dir_port", "or_port"],
        ],
    )
...
class NetworkStatusDocument(TorDocument):
    ...
    def is_reasonably_live(self):
        ...
        return (
            self.valid_after - timedelta(hours=24)
            <= datetime.utcnow()
            <= self.valid_until + timedelta(hours=24)
        )
    ...
    def apply_diff(self, diff):
        logger.info(
            "Apply network-status-diff for %s to %s",
            self.digest_sha3_256.hex(),
            diff.to_digest.lower(),
        )
