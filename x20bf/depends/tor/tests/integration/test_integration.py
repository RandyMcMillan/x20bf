...
logging.basicConfig(
    format="[%(asctime)s] [%(threadName)-16s] %(message)s", level=logging.DEBUG
)
...
def test_multi_threaded():
    auth_data = (
        ...
        if HS_BASIC_HOST and HS_BASIC_AUTH
        else None
    )
...
def test_select():
    ...
    events = {
        TorStream: {"data": Event(), "close": Event()},
